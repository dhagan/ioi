...
autoRefresh();
background();
$statement = "SELECT status, submitted_by, case_num, date_mod, time_mod, short_desc, prob_prod_link, prob_comp_link, updated_by, priority_type FROM problems WHERE assigned_to like '$user' AND status <> 'Closed' AND status <> 'Awaiting Bug Fix' AND status <> 'Awaiting Feature Request' ORDER BY " . $order_by;
$statement = "SELECT status, submitted_by, case_num, date_mod, time_mod, short_desc, prob_prod_link, prob_comp_link, updated_by, priority_type FROM problems WHERE assigned_to like '$user' AND status = 'Awaiting Bug Fix' ORDER BY " . $order_by if (param('showBugTickets'));
$statement = "SELECT status, submitted_by, case_num, date_mod, time_mod, short_desc, prob_prod_link, prob_comp_link, updated_by, priority_type FROM problems WHERE assigned_to like '$user' AND status = 'Awaiting Feature Request' ORDER BY " . $order_by if (param('showFeatureTickets'));
